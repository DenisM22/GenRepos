...
import type { ConfessionalDocument, PersonFromConfessionalDocument } from "@/app/types/models"
...
export default function DocumentPage() {
  ...
  const fetchDocument = async () => {
    try {
      const response = await confessionalDocumentApi.getById(params.id)
...
const PersonCard: React.FC<{ person: PersonFromConfessionalDocument; index: number }> = ({ person, index }) => {
  return (
      <Card className="h-full">
        ...
        <CardContent className="text-sm space-y-1.5">
          ...
          {person.place && (
              <p>
                <strong>Место:</strong> {person.place?.place}
              </p>
          )}
          {person.household && (
              <p>

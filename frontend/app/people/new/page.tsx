...
function AddPerson() {
    ...
    const validatePerson = (person: Person): string | null => {
        ...
        if (person.father && !person.father.firstName?.trim()) {
            return "Поле родственника (Отец) не может быть пустым";
        }

        if (person.mother && !person.mother.firstName?.trim()) {
            return "Поле родственника (Мать) не может быть пустым";
        }

        if (person.spouse && !person.spouse.firstName?.trim()) {
            return "Поле родственника (Супруг/Супруга) не может быть пустым";
        }

        if (person.children && person.children.length > 0) {
            for (let index = 0; index < person.children.length; index++) {
                const child = person.children[index];

                if (!child.firstName?.trim()) {
                    return `Поле родственника (Ребенок ${index + 1}) не может быть пустым`;
                }
            }
        }
